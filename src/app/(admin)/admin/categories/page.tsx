...
import {
  ...
  useDeleteCategory,
  useUpdateCategory,
} from "@/hooks/react-query/admin/category/use-mutation";

export default function Page() {
  const { data, isLoading: isLoadingData } = useAdminCategory();
  ...
  const { mutate: createTag } = useCreateCategory({
    onSuccess: (body) => {
      toast.success("Created", {
        description: `Category ${body.data.name} created successfully!`,
      });
      queryClient.invalidateQueries({ queryKey: ["get.admin.category"] });
    },
  });

  const { mutate: deleteTag, isPending: isLoadingDelete } = useDeleteCategory({
    onSuccess: (body) => {
      toast.success("Deleted", {
        description: `Category ${body.data.name} deleted successfully!`,
      });
      queryClient.invalidateQueries({ queryKey: ["get.admin.category"] });
    },
  });

  const { mutate: updateTag } = useUpdateCategory({
    id: editingTag?.id,
    onSuccess: (body) => {
      toast.success("Updated", {
        description: `Category ${body.data.name} updated successfully!`,
      });
      queryClient.invalidateQueries({ queryKey: ["get.admin.category"] });
    },
  });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[400px] font-mono">
              <DialogHeader>
                <DialogTitle>
                  {editingTag ? "Edit Categories" : "Create new Categories"}
                </DialogTitle>
                <DialogDescription>
                  {editingTag
                    ? "Update the categories information below."
                    : "Fill out the form below to create a new categories."}
                </DialogDescription>
              </DialogHeader>
        ...
        <Card className="bg-admin-card border-admin-border shadow-card">
          <CardHeader>
            ...
            <CardDescription>
              Most used categories in your content
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <CardDescription>
                    {data?.data?.length ?? 0} categories total
                  </CardDescription>
                </div>
              </div>
