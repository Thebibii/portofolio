...
import {
  useCreateProject,
  useUpdateProject,
} from "@/hooks/react-query/admin/projects/use-mutation";
...
export default function FormEditProject({
  ...
  const { data, isLoading } = useAdminProjectById({ id });
  ...
  const { mutate, isPending } = useUpdateProject({
    ...
    onSuccess: (body) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["get.admin.project", id] });
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      status: data?.data?.status || ProjectStatus.PLANNING,
      featured: data?.data?.featured ?? false,
      startDate: data?.data?.startDate ? new Date(data.startDate) : undefined,
      endDate: data?.data?.endDate ? new Date(data.endDate) : undefined,
    },
    values: {
      title: data?.data?.title || "",
      description: data?.data?.description || "",
